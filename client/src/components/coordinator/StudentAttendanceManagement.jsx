...
const StudentAttendanceManagement = () => {
  ...
  useEffect(() => {
    ...
    if (userId) {
      console.log('User loaded with ID:', userId, 'calling loadClasses');
      loadClasses();
    } else {
  ...
  const loadClasses = async () => {
    ...
    try {
      ...
      console.log('Loading classes for user ID:', userId);
      
      // Get classes where this coordinator is floor incharge
      const response = await handleApiResponse(
        async () => api.get(`/classes?floorIncharge=${userId}&isActive=true`),
        {
      ...
      {/* Header */}
        ...
        {classes.length === 0 && !loading && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
            ...
            <p className="text-gray-600">You are not assigned as floor coordinator for any classes.</p>
